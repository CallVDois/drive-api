...
import com.callv2.drive.infrastructure.member.model.MemberCreatedMessage;
import com.callv2.drive.infrastructure.messaging.listener.Listener;

@Component
public class MemberCreatedListener implements Listener<MemberCreatedMessage> {

    private static final Logger log = LogManager.getLogger(MemberCreatedListener.class);

    private final SynchronizeMemberUseCase synchronizeMemberUseCase;

    public MemberCreatedListener(final SynchronizeMemberUseCase synchronizeMemberUseCase) {
        this.synchronizeMemberUseCase = Objects.requireNonNull(synchronizeMemberUseCase);
    }

    @Override
    @RabbitListener(queues = "drive.member.created.queue")
    public void handle(final MemberCreatedMessage data) {

        final MemberCreatedMessage.Data eventData = data.data();
